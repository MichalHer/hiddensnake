...
class TestDES(TestCase):
    def test_apply_permutation(self):
        ...
        e = DESEncrypterCBC()
        permutation = e._DESEncrypterCBC__apply_permutation(test_bytes, perm_table)
    ...
    def test_shift_bytes(self):
        ...
        self.assertEqual(
            e._DESEncrypterCBC__shift_bytes(bytearray([255,0,0,0,0,0,0,0]), -4),
            bytearray([0b11110000,0,0,0,0,0,0,0b00001111])
        )

        self.assertEqual(
            e._DESEncrypterCBC__shift_bytes(bytearray([255,0,0,0,0,0,0,0]), -8),
            bytearray([0,0,0,0,0,0,0,255])
        )

        self.assertEqual(
            e._DESEncrypterCBC__shift_bytes(bytearray([255,0,0,0,0,0,0,0]), -7),
            bytearray([0b10000000,0,0,0,0,0,0,0b01111111])
        )

        self.assertEqual(
            e._DESEncrypterCBC__shift_bytes(bytearray([255,0,0,0]), -4),
    ...
    def test_prepare_key(self):
        ...
        e = DESEncrypterCBC()
        result = e._DESEncrypterCBC__prepare_key(key, -4)
        self.assertEqual(
            result,
